...
const AdminSidebar = () => {
    ...
    useEffect(() => {
        const activeItem = menuItems.findIndex(item => item.link === location.pathname);
            ...
            >
                ...
                <nav className="flex flex-col space-y-8 mx-8 mt-3">
                    {menuItems.map((item, index) => (
                        ...
                        >
                            <span className={`group-hover:text-white mr-2 ${activeLink === index ? 'text-white' : 'text-gray-400'}`}>
                                {item.icon}
                            </span>
